...
async def create_chat_completion(
    ...
    provider_response = await gateway_router.complete(
        model_alias=body.model,
        messages=[m.model_dump() for m in body.messages],
    ...
    usage = raw.usage
    completion_text = raw.choices[0].message.content or ""

    response_verdict = await guardrails.check_response(completion_text, context)
    ...
    cost = await cost_service.calculate(
        usage.prompt_tokens, usage.completion_tokens, provider_response.resolved_provider, provider_response.resolved_model
    )
    ...
    result = ChatCompletionResponse(
        ...
        choices=[
            ChatChoice(
                index=0,
                message=ChatMessage(role="assistant", content=response_verdict.masked_text or completion_text),
