...
**Explicitly out of scope today** (documented, not silently missing): encrypted-at-rest storage of credential values in the application database (only a reference/secret-name is stored); enforcement of budget and access-policy token ceilings at request time (both are currently visible/advisory only).
